...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=sulinys/multi-server:$GIT_SHA
kubectl set image deployments/client-deployment client=sulinys/multi-client:$GIT_SHA
kubectl set image deployments/worker-deployment worker=sulinys/multi-worker:$GIT_SHA
